...
var Scheduler = exports.Scheduler = function () {
    ...
    var createRunner = function (taskName, task) {
        return function () {
            runningTasks[taskName] = new Date();
            try {
                task.run();
            } catch (e) {
                log.error("Scheduler encountered exception while running", taskName, e);
            } finally {
    ...
    this.checkRunnableTasks = function () {
        var now = new Date();
        for each (let [taskName, task] in Iterator(tasks)) {
            try {
                ...
                spawn(createRunner(taskName, task));
